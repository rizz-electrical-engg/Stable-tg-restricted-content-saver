...
async def batch_link(_, message):
    ...
    if cl - cs > 100000:
        await app.send_message(message.chat.id, "Only 100000 messages allowed in batch size... Make sure your start and end message have difference less than 100000")
        return
            ...
            for i in range(int(s), int(l)):
                if user_id in users_loop and users_loop[user_id]:
                    ...
                    try:
                        ...
                        link = get_link(url)
                        await get_msg(userbot, user_id, msg.id, link, 0, message)
                        sleep_msg = await app.send_message(message.chat.id, "Sleeping for 15 seconds to avoid flood...")
